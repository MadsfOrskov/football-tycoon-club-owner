...
* **Committet:** kun `QA-REPORT.md` og `qa-probes.js`, kun på `nightly/qa`. Ingen ændring i prototypen eller `test-harness.js` i diffen (`git show --stat HEAD` viser to filer).
* **Om `nightly/qa`s historik:** min ordre sagde `git checkout -B nightly/qa origin/nightly/oejeblikke`, hvilket peger branchen et nyt sted hen — den gamle QA-branch havde ikke engang en fælles forfader, så den kunne ikke flettes uden også at trække en gammel prototype og en gammel harness med ind i min diff. Jeg valgte at holde diffen ren. **De tidligere rapporter ligger på `e3b7c253bc2e79057a4656a0df30347f57a68eff`** (nat 5+6, og gennem den nat 2 og 3+4). Jeg prøvede at bevare den som et tag; push af tags afvises med HTTP 403 i dette miljø. **Skriv SHA'en ned, hvis de gamle rapporter skal kunne findes igen.**
